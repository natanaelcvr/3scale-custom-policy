...
local _M = policy.new('JWTClaimCheck')
  ...
  for _, rule in ipairs(config.rules) do
    ...
    for _, condition in ipairs(rule.operations) do
      if condition.jwt_claim_type == "plain" then
        ...
        ngx.log(ngx.INFO, 'jwt-claim-check -> condition.jwt_claim ', condition.jwt_claim, ' =)')
      end
...
local function is_rule_denied_request(rule, context)
  ...
  ngx.log(ngx.INFO, 'jwt-claim-check -> using token ', context.jwt, ': for check conditions')
